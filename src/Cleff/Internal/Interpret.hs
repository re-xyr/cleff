...
class Handling e esSend | esSend -> e, e -> esSend where
...
sendHdlRef :: Handling e esSend => MemPtr InternalHandler e
sendHdlRef = let SendSite hdlRef _ = sendSite in hdlRef
{-# INLINE sendHdlRef #-}

-- | Get the send-site 'Env'.
sendEnv :: Handling e esSend => Env esSend
...
newtype InstHandling e esSend a = InstHandling (Handling e esSend => a)
...
instHandling :: forall e esSend a. (Handling e esSend => a) -> SendSite e esSend -> a
instHandling x = unsafeCoerce (InstHandling x :: InstHandling e esSend a)
{-# INLINE instHandling #-}

-- | The type of an effect handler, /i.e./ a function that interprets an effect @e@ from an arbitrary effect stack into
-- the effect stack @es@.
type Handler e es = forall esSend. Handling e esSend => e (Eff esSend) ~> Eff es
...
mkInternalHandler ref es handle = InternalHandler \eff -> Eff \esSend ->
  unEff (instHandling handle (SendSite ref esSend) eff) (Mem.update esSend es)
...
reinterpretN handle m = Eff \es ->
  let (# ref, es' #) = Mem.alloca es
  in unEff m $ Mem.append ref (mkInternalHandler ref es' handle) $ Mem.adjust (Env.drop @es') es'
...
imposeN handle m = Eff \es ->
  let (# ref, es' #) = Mem.alloca es
  in unEff m $ Mem.replace ref (mkInternalHandler ref es' handle) $ Mem.adjust (Env.drop @es') es'
...
translateN trans m = Eff \es ->
  ...
  in unEff m $ Mem.append ref handler $ Mem.adjust (Env.drop @es') es'
...
--     ('runThere' . use)
-- @
runThere :: Handling e esSend => Eff esSend ~> Eff es
...
-- @
runHere :: Handling e esSend => Handler e es -> Eff esSend ~> Eff es
runHere handle m = Eff \es -> unEff m $
  Mem.write sendHdlRef (mkInternalHandler sendHdlRef es handle) $ Mem.update es sendEnv
...
withUnliftIO :: Handling e esSend => ((Eff esSend ~> IO) -> Eff es a) -> Eff es a
...
withLiftIO :: Handling e esSend => ((IO ~> Eff esSend) -> IO a) -> IO a
...
withLiftEff :: Handling e esSend => ((Eff es ~> Eff esSend) -> Eff es a) -> Eff es a
